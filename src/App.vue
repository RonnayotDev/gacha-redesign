<template>
  <div class="relative w-full h-screen bg-gray-900 overflow-hidden text-white font-sans selection:bg-blue-500 selection:text-white">
    ...
    <div class="absolute top-4 right-4 md:top-6 md:right-6 z-[100] flex items-center gap-3 bg-black rounded-full px-4 py-2 md:px-6 md:py-3 border border-gray-800 shadow-xl">
        <div class="w-6 h-6 md:w-8 md:h-8 flex items-center justify-center">
          <img src="../assets/img/Lucky_wheel.png" alt="Icon" class="w-full h-full object-contain filter drop-shadow-md" />
        </div>
    ...
    <div class="relative z-10 w-full h-full flex flex-col">
      <div class="flex-1 flex flex-col items-center justify-center px-4 transition-all duration-500" :class="showWheelModal ? 'blur-sm scale-95 opacity-50' : ''">
        
        <div class="text-center space-y-4 mb-8">
          <h1 class="text-5xl md:text-7xl font-display font-black tracking-tighter text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.5)] flex justify-center items-center">
          ...
          <p class="text-blue-300/80 text-sm md:text-base italic max-w-2xl mx-auto">
             Lorem ipsum dolor sit amet, consectetur adipisicing elit. Dolorem iure explicabo fuga, quasi, fugit quae dolore dolores iusto, magni molestiae distinctio. 
          </p>
        </div>
          ...
          <div v-for="(item, index) in targetWord" :key="index" class="group relative">
            ...
            >
              <img 
                ...
                class="w-full h-full object-contain p-1 transition-all duration-500"
                :class="item.collected ? 'opacity-100 scale-100' : 'opacity-0 scale-50'"
              />
              <span v-else class="text-4xl md:text-6xl font-balloon transition-all duration-500"
                :class="[
                  item.collected ? 'opacity-100 scale-100' : 'opacity-0 scale-50',
                  index % 3 === 0 ? 'text-balloon-blue' : index % 3 === 1 ? 'text-balloon-purple' : 'text-balloon-cyan'
                ]"
              >
                {{ item.char }}
              </span>
        ...
        <div class="mt-8 text-center">
          ...
          <p v-if="remainingSpins === 0 && !showWheelModal && !showResultModal" class="text-red-400 mt-4 text-sm">
            * สิทธิ์หมุนของคุณหมดแล้ว
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-if="showWheelModal" class="fixed inset-0 z-50 flex items-center justify-center  backdrop-blur-sm p-4 transition-all duration-500">
      
      <div class="relative flex flex-col items-center max-w-2xl w-full">
        <h2 class="text-3xl md:text-5xl font-display font-black text-white mb-6 md:mb-10 drop-shadow-[0_0_15px_rgba(59,130,246,0.8)] tracking-wider">
        ...
        <div class="relative w-[380px] h-[380px] md:w-[600px] md:h-[600px] flex items-center justify-center">
          ...
          <div 
            class="relative z-10 w-full h-full rounded-full border-[8px] md:border-[12px] border-[#3b82f6] shadow-[0_0_50px_rgba(37,99,235,0.5)] overflow-hidden transition-transform duration-[4000ms] ease-out-quart"
          ...
          >
            ...
            <div 
              ...
              :key="'bg-'+reward.id"
              class="absolute top-0 left-1/2 w-1/2 h-[50%] origin-bottom-left border-l border-white/10"
              :style="{ 
            ...
            <div class="absolute inset-0 w-full h-full pointer-events-none">
              <div 
                 v-for="(reward, index) in rewards" 
                 :key="'content-'+reward.id"
                 class="absolute top-1/2 left-1/2 w-0 h-0 flex justify-center items-center"
                 :style="{
                    transform: `rotate(${index * 45 + 22.5}deg)` 
                 }"
              >
                 <div class="flex flex-col items-center justify-center text-center transform -translate-y-[110px] md:-translate-y-[180px] w-32 md:w-40">
                     ...
                     <div class="text-center bg-black/30 backdrop-blur-[2px] rounded-lg px-2 py-1 w-full flex flex-col items-center border border-white/5">
                        <span class="block text-[10px] md:text-sm font-bold text-white leading-tight shadow-black drop-shadow-md break-words max-w-full">
                           {{ reward.name }}
                        </span>
                        <span class="block text-[9px] md:text-[10px] text-cyan-300 mt-0.5 font-medium" v-if="reward.stock > 0">
                           x{{ reward.stock }}
                        </span>
            ...
            <div class="absolute inset-0 m-auto w-[30%] h-[30%] rounded-full bg-gradient-to-b from-[#1e293b] to-[#0f172a] border-[4px] md:border-[6px] border-[#3b82f6] shadow-[inset_0_0_20px_rgba(0,0,0,0.8)] z-20 flex items-center justify-center">
                 <div class="w-full h-full rounded-full bg-blue-500/10 flex items-center justify-center animate-pulse-slow">
          ...
          >
            ...
            <span class="relative font-black text-xl md:text-3xl italic tracking-tighter text-white drop-shadow-md group-hover:animate-pulse">
              SPIN
            </span>
          </button>
        </div>
        
        <button v-if="!isSpinning" @click="showWheelModal = false" class="mt-4 text-gray-500 text-sm underline hover:text-white transition-colors">
          กลับไปหน้าหลัก
        </button>
      </div>
    </div>
    ...
    <div v-if="showResultModal" class="fixed inset-0 z-[60] flex items-center justify-center bg-black animate-fade-in overflow-hidden">
       <div class="absolute inset-0 transition-colors duration-1000 opacity-60"
            :class="[
                wonReward?.theme === 'red' ? 'bg-red-900' : 
                wonReward?.theme === 'green' ? 'bg-green-900' :
                wonReward?.theme === 'blue' ? 'bg-blue-900' : 'bg-gray-800'
            ]"
       ></div>
       ...
       <div class="relative z-10 flex flex-col items-center justify-center text-center w-full max-w-4xl px-4">
          <div class="relative mb-8">
             ...
             <p class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 font-script text-6xl md:text-6xl opacity-90 whitespace-nowrap mt-6"
                :class="[
                    wonReward?.theme === 'red' ? 'text-red-500' : 
                    wonReward?.theme === 'green' ? 'text-green-400' :
                    wonReward?.theme === 'blue' ? 'text-blue-400' : 'text-gray-300'
                ]"
          ...
          <p class="text-gray-200 text-lg md:text-xl font-light mb-10">
              Wow! You've just won a <span class="font-bold text-white">{{ wonReward?.name }}</span>
          </p>
          
          <div class="relative flex items-center justify-center w-full py-10 mb-8">
             <div class="absolute w-[400px] h-[400px] rounded-full border-2 border-white/10 scale-100 animate-pulse-slow"></div>
             <div class="absolute w-[300px] h-[300px] rounded-full border border-white/20 scale-90"></div>
             
             <div class="absolute w-64 h-64 rounded-full blur-[80px]" 
                  :class="[wonReward?.theme === 'red' ? 'bg-red-600/60' : wonReward?.theme === 'green' ? 'bg-green-500/60' : wonReward?.theme === 'blue' ? 'bg-blue-500/60' : 'bg-white/30']">
             </div>
...
const handleSlotClick = (index) => {
  if (!targetWord[index].collected) {
    targetWord[index].collected = true; // change status to collected
  }
};
...
const spinTheWheel = () => {
  ...
  isSpinning.value = true; // disabled button
  ...
  const totalWeight = availableRewards.reduce((sum, item) => sum + item.weight, 0);
  ...
  const rewardIndex = rewards.value.findIndex(r => r.id === selectedReward.id);
  const segmentAngle = 360 / rewards.value.length; 
  

  const stopAngle = 3600 - (rewardIndex * segmentAngle) - (segmentAngle / 2);
  ...
  setTimeout(() => {
    ...
    // decrease stock
    const realRewardIndex = rewards.value.findIndex(r => r.id === selectedReward.id);
    if(realRewardIndex !== -1) {
       rewards.value[realRewardIndex].stock--;
    }

    showResultModal.value = true; // เปิดหน้าต่างแสดงผลลัพธ์
  }, 4000);
};
...
.text-balloon-blue { color: #60a5fa; }
.text-balloon-purple { color: #a855f7; }
.text-balloon-cyan { color: #22d3ee; }


.sunburst {
  background: repeating-conic-gradient(from 0deg, rgba(0, 0, 0, 0.8) 0deg 10deg, rgba(30, 58, 138, 0.2) 10deg 20deg);
}
...
.bg-radial-gradient-center {
    background-image: radial-gradient(circle, transparent 0%, rgba(0,0,0,0.4) 60%, rgba(0,0,0,0.9) 100%);
}
